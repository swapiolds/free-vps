...
DEPLOY_RAM, DEPLOY_CPU, DEPLOY_DISK = range(10, 13)
...
async def cmd_deploy_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    username = update.effective_user.first_name or update.effective_user.username or str(user_id)
    ...
    if invites < 20 and not is_admin(user_id):
        await update.message.reply_text(f"❌ <b>Need 20 invites.</b> Current: {invites}/20", parse_mode=ParseMode.HTML)
        return ConversationHandler.END
        
    if count_user_vps(user_id) >= 1:
        await update.message.reply_text("❌ <b>Max 1 VPS per user.</b>", parse_mode=ParseMode.HTML)
        return ConversationHandler.END
        
    await update.message.reply_text("🖥 <b>How many GB RAM do you want? (e.g. 2, 4, 8)</b>", parse_mode=ParseMode.HTML, reply_markup=ReplyKeyboardMarkup([["2", "4", "8"], ["Cancel"]], resize_keyboard=True))
    return DEPLOY_RAM
...
async def cancel_deploy(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("❌ <b>Deployment cancelled.</b>", reply_markup=get_main_menu_keyboard(), parse_mode=ParseMode.HTML)
    return ConversationHandler.END
...
async def cmd_deploy_disk(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("⏳ <b>Initializing VPS locally...</b>", reply_markup=get_main_menu_keyboard(), parse_mode=ParseMode.HTML)
    
    vps_id = str(uuid.uuid4())[:8]
    hostname = f"{VPS_HOSTNAME}-{user_id}"
    container_name = f"vps-{user_id}-{vps_id}"
    
    ram = context.user_data['deploy_ram']
    cpu = context.user_data['deploy_cpu']
    disk = context.user_data['deploy_disk']
    ...
    if ssh_line:
        ...
        try:
            await msg.edit_text("❌ <b>Failed to create VPS locally or timed out. Please check logs.</b>", parse_mode=ParseMode.HTML)
        except: pass
...
def main():
    ...
    deploy_conv = ConversationHandler(
        ...
        states={
            DEPLOY_RAM: [MessageHandler(filters.TEXT & ~filters.COMMAND, cmd_deploy_ram)],
            DEPLOY_CPU: [MessageHandler(filters.TEXT & ~filters.COMMAND, cmd_deploy_cpu)],
            DEPLOY_DISK: [MessageHandler(filters.TEXT & ~filters.COMMAND, cmd_deploy_disk)]
        },
        fallbacks=[CommandHandler("cancel", cancel_deploy), MessageHandler(filters.Regex("🚀"), cmd_deploy_start)],
        allow_reentry=True
    )
